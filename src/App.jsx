...
function App() {
  ...
  return (
    <div className={`min-h-screen bg-white text-gray-900 font-sans transition-colors duration-700`}>
            ...
            >
              ...
              <div className="space-y-6">
                <input 
                  ...
                  className="w-full py-4 border-b border-gray-200 outline-none focus:border-[#C5A059] transition-all text-lg font-light"
                />
                <textarea 
                  placeholder={isAr ? 'رسالتك' : 'How can we help?'} 
                  onChange={(e) => setMessage(e.target.value)}
                  className="w-full py-4 border-b border-gray-200 h-28 resize-none outline-none focus:border-[#C5A059] transition-all font-light"
                />
              </div>
              <div className="flex items-center gap-8 mt-12">
                <button onClick={() => setShowModal(false)} className="text-[10px] font-bold uppercase tracking-widest text-gray-400 hover:text-black transition-colors">
                  {t.back}
                </button>
                <button 
                  onClick={envoyerWhatsApp}
                  className="flex-1 py-5 bg-black text-white rounded-full text-[10px] font-bold uppercase tracking-widest shadow-xl hover:bg-[#C5A059] transition-all"
                >
                  {t.send}
                </button>
              </div>
      ...
      <footer className="pb-16 text-center text-[9px] text-gray-400 tracking-[0.5em] uppercase font-light">
        Excellence Française • Kuwait City • 2026
      </footer>
    </div>
  );
}
